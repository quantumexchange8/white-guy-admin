...
const internalNewsActionHistories = ref();
...
const getInternalNewsActionHistory = async () => {
    try {
        isLoading.value = true;
        const response = await axios.get(`/crm/internalNews/getInternalNewsLogEntries?id=` + props.internal_news.id);

        internalNewsActionHistories.value = response.data;
    } catch (error) {
...
// watch(() => props.internalNewsActionHistories, (user) => {
//     form.user_id = props.internalNewsActionHistories.id
//     form.name = props.internalNewsActionHistories.name
//     form.email = props.internalNewsActionHistories.email
//     form.phone = props.internalNewsActionHistories.phone
...
<template>
    <div class="flex flex-col gap-6 items-center self-stretch py-4 md:py-6 md:gap-8">
        ...
        <Accordion multiple class="w-full max-h-[600px] overflow-auto">
            ...
            <AccordionPanel
                v-else
                v-for="(note, index) in internalNewsActionHistories" 
                :key="note.id" 
                :value="index"
            >
                <div class="w-full flex gap-2">
                    ...
                    <div
                        ...
                        :class="{
                            'py-2 border-b': index !== 0 && index !== internalNewsActionHistories.length - 1,
                            'pb-2 border-b': index === 0,
                            'pt-2': index === internalNewsActionHistories.length - 1
                        }"
                    >
        ...
        <div v-if="!isLoading && internalNewsActionHistories?.length <= 0">
