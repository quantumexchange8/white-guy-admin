...
const items = ref([
    {
        ...
        icon: h(IconIdBadge2),
        command: () => {
            window.location.href = `/crm/notification/detail/${props.notification.id}`;
        },
    },
    {
        ...
        icon: h(IconTrashX),
        command: () => {
            requireConfirmation('delete_notification')
        },
    },
]);
...
const requireConfirmation = (action_type) => {
    const messages = {
        delete_lead: {
            ...
            color: 'error',
            icon: h(IconTrashX),
            header: trans('public.delete_notification'),
            message: trans('public.delete_notification_desc'),
            cancelButton: trans('public.cancel'),
            acceptButton: trans('public.delete'),
            action: () => {
                router.visit(route('notification.deleteNotification'), {
                    method: 'delete',
                    data: {
