...
import PrivateMessageActionHistory from "@/Pages/CRM/PrivateMessages/Partials/PrivateMessageActionHistory.vue";
...
const items = ref([
    {
        label: 'private_message_action_history',
        icon: h(IconIdBadge2),
        command: () => {
            visible.value = true;
            dialogType.value = 'private_message_action_history';
            title.value = 'private_message_action_history';
        },
    },
    {
        label: 'delete_private_message',
        icon: h(IconTrashX),
        command: () => {
            requireConfirmation('delete_private_message')
        },
    },
]);
...
const requireConfirmation = (action_type) => {
    const messages = {
        delete_private_message: {
            ...
            color: 'error',
            icon: h(IconTrashX),
            header: trans('public.delete_private_message'),
            message: trans('public.delete_private_message_desc'),
...
<template>
    ...
    >
        <template v-if="dialogType === 'private_message_action_history'">
            <PrivateMessageActionHistory 
                :private_message="props.private_message"
            />
        </template>
    </Dialog>
