...
const privateMessageHistories = ref();
...
const getPrivateMessageHistory = async () => {
    try {
        isLoading.value = true;
        const response = await axios.get(`/crm/privateMessage/getPrivateMessageLogEntries?id=` + props.private_message.id);

        privateMessageHistories.value = response.data;
    } catch (error) {
...
// watch(() => props.privateMessageHistories, (user) => {
//     form.user_id = props.privateMessageHistories.id
//     form.name = props.privateMessageHistories.name
//     form.email = props.privateMessageHistories.email
//     form.phone = props.privateMessageHistories.phone
...
<template>
    <div class="flex flex-col gap-6 items-center self-stretch py-4 md:py-6 md:gap-8">
        ...
        <Accordion multiple class="w-full max-h-[600px] overflow-auto">
            ...
            <AccordionPanel
                v-else
                v-for="(note, index) in privateMessageHistories" 
                :key="note.id" 
                :value="index"
            >
                <div class="w-full flex gap-2">
                    ...
                    <div
                        ...
                        :class="{
                            'py-2 border-b': index !== 0 && index !== privateMessageHistories.length - 1,
                            'pb-2 border-b': index === 0,
                            'pt-2': index === privateMessageHistories.length - 1
                        }"
                    >
        ...
        <div v-if="!isLoading && privateMessageHistories?.length <= 0">
