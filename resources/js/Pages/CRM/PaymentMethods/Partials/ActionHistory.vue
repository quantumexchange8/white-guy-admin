...
const getPaymentMethodActionHistory = async () => {
    try {
        isLoading.value = true;
        const response = await axios.get(`/crm/paymentMethod/getPaymentMethodLogEntries?id=` + props.payment_method.id);

        paymentMethodActionHistories.value = response.data;
    } catch (error) {
...
// watch(() => props.paymentMethodActionHistories, (user) => {
//     form.user_id = props.paymentMethodActionHistories.id
//     form.name = props.paymentMethodActionHistories.name
//     form.email = props.paymentMethodActionHistories.email
//     form.phone = props.paymentMethodActionHistories.phone
...
<template>
    <div class="flex flex-col gap-6 items-center self-stretch py-4 md:py-6 md:gap-8">
        ...
        <Accordion multiple class="w-full max-h-[600px] overflow-auto">
            ...
            <AccordionPanel
                v-else
                v-for="(note, index) in paymentMethodActionHistories" 
                :key="note.id" 
                :value="index"
            >
                <div class="w-full flex gap-2">
                    ...
                    <div
                        ...
                        :class="{
                            'py-2 border-b': index !== 0 && index !== paymentMethodActionHistories.length - 1,
                            'pb-2 border-b': index === 0,
                            'pt-2': index === paymentMethodActionHistories.length - 1
                        }"
                    >
