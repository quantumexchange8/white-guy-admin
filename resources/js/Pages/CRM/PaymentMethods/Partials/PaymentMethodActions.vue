...
import ActionHistory from "@/Pages/CRM/PaymentMethods/Partials/ActionHistory.vue";
...
const items = ref([
    {
        label: 'payment_method_action_history',
        icon: h(IconIdBadge2),
        command: () => {
            visible.value = true;
            dialogType.value = 'payment_method_action_history';
            title.value = 'payment_method_action_history';
        },
    },
    {
        ...
        icon: h(IconTrashX),
        command: () => {
            requireConfirmation('delete_payment_method')
        },
    },
]);
...
const requireConfirmation = (action_type) => {
    const messages = {
        delete_payment_method: {
            ...
            color: 'error',
            icon: h(IconTrashX),
            header: trans('public.delete_payment_method'),
            message: trans('public.delete_payment_method_desc'),
...
<template>
    ...
    >
        <template v-if="dialogType === 'payment_method_action_history'">
            <ActionHistory 
