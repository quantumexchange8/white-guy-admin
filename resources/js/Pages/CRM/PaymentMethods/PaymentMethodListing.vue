...
import PaymentMethodActions from "@/Pages/CRM/PaymentMethods/Partials/PaymentMethodActions.vue";
...
const getResults = async () => {
    ...
    try {
        // Define the base URL
        let url = `/crm/paymentMethod/getPaymentMethods?rows=${rows.value}&page=${page.value}`;
...
<template>
    <AuthenticatedLayout :title="`${$t('public.payment_methods')}`">
        <div class="flex flex-col justify-center items-center px-3 py-5 self-stretch rounded-lg bg-white dark:bg-gray-900 shadow-card md:p-6 md:gap-6">
            <div class="flex flex-col pb-3 gap-3 items-center self-stretch md:flex-row md:gap-0 md:justify-between md:pb-0">
                <span class="text-gray-950 dark:text-gray-100 font-semibold self-stretch md:self-auto">{{ $t('public.payment_methods') }}</span>
                <div class="flex flex-col gap-3 items-center self-stretch md:flex-row md:gap-5">
                    <Button variant="primary-outlined" @click="exportXLSX()" :disabled="paymentMethods.length <= 0" class="w-full md:w-auto">
            ...
            >                
                ...
                <template v-if="paymentMethods?.length > 0">
                    <Column field="title" :header="$t('public.title')" class="w-3/4 md:w-[20%] max-w-0 px-3">
                    ...
                    <Column field="sites_count" :header="`${$t('public.sites_count')}`" class="hidden md:table-cell w-[20%] max-w-0">
                        <template #body="slotProps">
                            <div class="text-gray-950 dark:text-gray-100 text-sm">
                                {{ slotProps.data.sites_count }}
                            </div>
                        </template>
                    </Column>
                    <Column field="bank_name" :header="`${$t('public.cryptocurrency')}`" class="hidden md:table-cell w-[20%] max-w-0">
                        <template #body="slotProps">
                            <div class="text-gray-950 dark:text-gray-100 text-sm">
                                {{ slotProps.data.bank_name ? slotProps.data.bank_name : '-' }}
                            </div>
                        </template>
                    </Column>
                    <Column field="account_name" :header="`${$t('public.network')}`" class="hidden md:table-cell w-[20%] max-w-0">
                        <template #body="slotProps">
                            <div class="text-gray-950 dark:text-gray-100 text-sm">
                                {{ slotProps.data.account_name ? slotProps.data.account_name : '-' }}
                            </div>
                        </template>
                    </Column>
    ...
    <Dialog v-model:visible="visible" modal :header="$t('public.payment_method_details')" class="dialog-xs md:dialog-md">
        <div class="flex flex-col justify-center items-center gap-3 self-stretch pt-4 md:pt-6">
            <div class="flex flex-col items-center p-3 gap-3 self-stretch bg-gray-50 dark:bg-gray-700">
                ...
                <div class="w-full flex flex-col items-start gap-1 md:flex-row">
                    <span class="w-full max-w-[200px] truncate text-gray-500 dark:text-gray-300 text-sm">{{ $t('public.cryptocurrency') }}</span>
                    <span class="w-full text-gray-950 dark:text-gray-100 text-sm font-medium break-words">{{ data?.bank_name ? data?.bank_name : '-' }}</span>
                </div>
                <div class="w-full flex flex-col items-start gap-1 md:flex-row">
                    <span class="w-full max-w-[200px] truncate text-gray-500 dark:text-gray-300 text-sm">{{ $t('public.network') }}</span>
                    <span class="w-full text-gray-950 dark:text-gray-100 text-sm font-medium break-words">{{ data?.account_name ? data?.account_name : '-' }}</span>
                </div>
                <div class="w-full flex flex-col items-start gap-1 md:flex-row">
                    <span class="w-full max-w-[200px] truncate text-gray-500 dark:text-gray-300 text-sm">{{ $t('public.wallet_address') }}</span>
                    <span class="w-full text-gray-950 dark:text-gray-100 text-sm font-medium break-all" @click="copyToClipboard(data.account_number)">
                        {{ data?.account_number ? data?.account_number : '-' }}
                        <IconCopy 
                            v-if="data?.account_number"
                            size="20" 
                            stroke-width="1.25" 
                            class="text-gray-500 dark:text-gray-300 inline-block cursor-pointer grow-0 shrink-0" 
                            v-tooltip.top="$t(`public.${tooltipText}`)" 
                            @click="copyToClipboard(data?.account_number)"
                        />
                    </span>
                </div>
                <div class="w-full flex flex-col items-start gap-1 md:flex-row">
                    <span class="w-full max-w-[200px] truncate text-gray-500 dark:text-gray-300 text-sm">{{ $t('public.sites_count') }}</span>
                    <span class="w-full text-gray-950 dark:text-gray-100 text-sm font-medium break-words">{{ data?.sites_count ? data?.sites_count : '-' }}</span>
                </div>
            </div>            
        </div>
    </Dialog>
