...
const paymentSubmissionActionHistories = ref();
...
const getPaymentSubmissionActionHistory = async () => {
    try {
        isLoading.value = true;
        const response = await axios.get(`/crm/paymentSubmission/getPaymentSubmissionLogEntries?id=` + props.payment_submission.id);

        paymentSubmissionActionHistories.value = response.data;
    } catch (error) {
...
getPaymentSubmissionActionHistory();
...
// watch(() => props.paymentSubmissionActionHistories, (user) => {
//     form.user_id = props.paymentSubmissionActionHistories.id
//     form.name = props.paymentSubmissionActionHistories.name
//     form.email = props.paymentSubmissionActionHistories.email
//     form.phone = props.paymentSubmissionActionHistories.phone
...
<template>
    <div class="flex flex-col gap-6 items-center self-stretch py-4 md:py-6 md:gap-8">
        ...
        <Accordion multiple class="w-full max-h-[600px] overflow-auto">
            ...
            <AccordionPanel
                v-else
                v-for="(note, index) in paymentSubmissionActionHistories" 
                :key="note.id" 
                :value="index"
            >
                <div class="w-full flex gap-2">
                    ...
                    <div
                        ...
                        :class="{
                            'py-2 border-b': index !== 0 && index !== paymentSubmissionActionHistories.length - 1,
                            'pb-2 border-b': index === 0,
                            'pt-2': index === paymentSubmissionActionHistories.length - 1
                        }"
                    >
        ...
        <div v-if="!isLoading && paymentSubmissionActionHistories?.length <= 0">
