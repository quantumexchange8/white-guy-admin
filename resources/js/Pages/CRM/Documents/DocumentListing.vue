...
import DocumentActions from "@/Pages/CRM/Documents/Partials/DocumentActions.vue";
...
const getResults = async () => {
    ...
    try {
        // Define the base URL
        let url = `/crm/document/getDocuments?rows=${rows.value}&page=${page.value}`;
...
<template>
    <AuthenticatedLayout :title="`${$t('public.documents')}`">
        <div class="flex flex-col justify-center items-center px-3 py-5 self-stretch rounded-lg bg-white dark:bg-gray-900 shadow-card md:p-6 md:gap-6">
            <div class="flex flex-col pb-3 gap-3 items-center self-stretch md:flex-row md:gap-0 md:justify-between md:pb-0">
                <span class="text-gray-950 dark:text-gray-100 font-semibold self-stretch md:self-auto">{{ $t('public.documents') }}</span>
                <div class="flex flex-col gap-3 items-center self-stretch md:flex-row md:gap-5">
                    <Button variant="primary-outlined" @click="exportXLSX()" :disabled="documents.length <= 0" class="w-full md:w-auto">
            ...
            >                
                ...
                <template v-if="documents?.length > 0">
                    ...
                    <Column field="document" :header="`${$t('public.document')}`" class="hidden md:table-cell w-[60%] max-w-0">
                        <template #body="slotProps">
                            <div class="text-gray-950 dark:text-gray-100 text-sm">
    ...
    <Dialog v-model:visible="visible" modal :header="$t('public.document_details')" class="dialog-xs md:dialog-md">
        <div class="flex flex-col justify-center items-center gap-3 self-stretch pt-4 md:pt-6">
            <div class="flex flex-col items-center p-3 gap-3 self-stretch bg-gray-50 dark:bg-gray-700">
                ...
                <div class="w-full flex flex-col items-start gap-1 md:flex-row">
                    <span class="w-full max-w-[200px] truncate text-gray-500 dark:text-gray-300 text-sm">{{ $t('public.document') }}</span>
                    <span class="w-full text-gray-950 dark:text-gray-100 text-sm font-medium break-all">{{ data?.document ? data?.document : '-' }}</span>
                </div>
            </div>            
        </div>
    </Dialog>
