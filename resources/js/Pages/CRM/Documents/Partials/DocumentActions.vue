...
import DocumentActionHistory from "@/Pages/CRM/Documents/Partials/DocumentActionHistory.vue";
...
const items = ref([
    {
        label: 'document_action_history',
        icon: h(IconIdBadge2),
        command: () => {
            visible.value = true;
            dialogType.value = 'document_action_history';
            title.value = 'document_action_history';
        },
    },
    {
        ...
        icon: h(IconTrashX),
        command: () => {
            requireConfirmation('delete_document')
        },
    },
]);
...
const requireConfirmation = (action_type) => {
    const messages = {
        delete_document: {
            ...
            color: 'error',
            icon: h(IconTrashX),
            header: trans('public.delete_document'),
            message: trans('public.delete_document_desc'),
...
<template>
    ...
    >
        <template v-if="dialogType === 'document_action_history'">
