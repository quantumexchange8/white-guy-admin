...
const userTimezone = usePage().props.timezone;
...
const getResults = async () => {
    try {
        ...
        const response = await axios.get(`/crm/application/getApplicationLogEntries?id=` + props?.application?.id);
...
// watch(() => props.logs, (user) => {
//     form.user_id = props.logs.id
//     form.name = props.logs.name
//     form.email = props.logs.email
//     form.phone = props.logs.phone
...
const submitForm = () => {
    ...
    form.post(route('application.updateMemberInfo'), {
        onSuccess: () => {
            visible.value = false;
        },
    });
    ...
    <div class="w-full flex flex-col items-center p-3 gap-3 self-stretch rounded-lg bg-white dark:bg-gray-800 shadow-card md:px-6 md:py-5">
        <div class="flex flex-col justify-center items-center gap-2 self-stretch">
            <div class="flex justify-between items-start self-stretch">
                <span class="w-full text-gray-950 dark:text-white font-bold text-xxl break-words">{{ $t('public.history') }}</span>
                <!-- <Button
                    type="button"
                    iconOnly
                    size="base"
            ...
            >
                <div class="w-full flex gap-2">
                    ...
                    <div
                        ...
                        :class="{
                            'py-2 border-b': index !== 0 && index !== logs.length - 1,
                            'pb-2 border-b': index === 0,
                            'pt-2': index === logs.length - 1
                        }"
                    >
                        <div class="flex flex-col items-center pt-2 gap-1">
                            <h3 class="self-stretch text-gray-950 dark:text-gray-100 font-semibold">{{ `[${$t('public.system')}]: ` + (log.action === 0 ? 'Newly created' : 'Updated') }}</h3>
                            <span class="self-stretch text-sm text-gray-500 dark:text-gray-400">{{ log.created_at }}</span>
                        </div>
                        <AccordionHeader class="px-2 text-start">{{ `${$t('public.log_details')}&nbsp;(${log.id})` }}</AccordionHeader>
                        <AccordionContent>
                            <div class="w-full flex flex-col items-center p-2">
                                <!-- Loop through all changes dynamically -->
                                <div v-for="(change, key) in extractChanges(log.changes)" :key="key" class="w-full flex flex-col items-start justify-center gap-1">
